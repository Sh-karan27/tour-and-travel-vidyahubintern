...
const Services = () => {
    ...
    const getCardClass = (index) => {
        const position = (index - currentIndex + serviceData.length) % serviceData.length;
        ...
        <div className="w-11/12 h-screen mx-auto flex flex-col">
            ...
            <div className="flex-1 flex items-center justify-center relative overflow-hidden">
                {serviceData.map((service, index) => (
                    <div
                        key={service.id}
                        className={`absolute w-60 h-80 p-4 bg-gradient-to-br from-white to-gray-100 shadow-lg rounded-xl transition-all duration-500 ease-in-out ${getCardClass(index)} card-glow`}
                    >
